...
begin
  ...
  with DesU.qrAbra do begin
    ...
    { TODO tyhle kontroly d·t jinam, resp. je v˘bec pot¯eba? m·m fakturu na Firmu, proË teÔ ¯eöit MULTIPLE?
    if Fields[0].AsString = 'MULTIPLE' then begin
      Result := Format('%s (%s): VÌce z·kaznÌk˘ pro kÛd %s.', [self.Firm.Name, self.VS, self.Firm.AbraCode]);
      Close;
      Exit;  // tohle ale nestaËÌ, je t¯eba vyskoËit z celÈho p¯evodu faktury, nebo to udÏlat nÏjak jinak spr·vnÏ
             // by se mÏlo zkontrolovat p¯i naËÌt·nÌ faktur a v˘bec nepouötÏt do p¯evodu
    end;
    }
  ...
  Saldo := Saldo + self.CastkaZaplacenoZakaznikem;          // Saldo je po splatnosti (SaldoPo), je-li faktura uû zaplacena, p¯iËte se platba
  Zaplatit := self.Castka - Saldo;          // Celkem k ˙hradÏ = Celkem za fakt. obdobÌ - Z˘statek minul˝ch obdobÌ(saldo)
  ...
  //pro staröÌ osmimÌstn· ËÌsla smluv se p¯idajÌ dvÏ nuly na zaË·tek
  {
  ...
  reportAry['SS2'] := slozenkaSS; // SS2 se liöÌ od SS tak, ûe m· 8 mÌst. SS m· 6 mÌst (zleva jsou vûdy p¯idanÈ nuly)

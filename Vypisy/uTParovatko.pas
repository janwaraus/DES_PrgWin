  ...
  begin
    ...
    begin
      ...
      if Platba.getPocetPredchozichPlatebZeStejnehoUctu() >= 3 then begin
        ...
        vytvorPDPar(Platba, Doklad, Castka, 'p¯epl. | ' + Platba.VS + ' |', false);
        Platba.zprava := 'nezn·m˝ p¯ep. ' + FloatToStr(Castka) + ' KË';
...
begin
  ...
  self.odparujPlatbu(Platba); //nenÌ vlastnÏ uû pot¯eba, protoûe vûdy p·rujeme nanovo vöechny Platby od zaË·tku do konce
  ...
  begin //platba je kredit (plusov·)
    ...
    begin
      Platba.zprava := 'nov˝/nezn·m˝ ˙Ëet - ' + Platba.zprava;
...
begin
  ...
  begin
    ...
    if iPDPar.vazbaNaDoklad AND Assigned(iPDPar.Doklad) then
      Result := Result + 'Na doklad ' + iPDPar.Doklad.ID + ' nap·rov·no ' + FloatToStr(iPDPar.CastkaPouzita) + ' KË ';

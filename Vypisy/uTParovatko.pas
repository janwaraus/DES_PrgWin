...
begin
  ...
  begin //platba je kredit (plusov·)
    ...
    // hled·m doklad, jehoû nezaplacen· Ë·stka se rovn· Ë·stce platby
    ...
    // pokud se nenaöel doklad, kter˝ by ölo p¯esnÏ zaplatit, vezmu si vöechny nezaplacenÈ
...
begin
  ...
  begin
    ...
    //newBo.rowItem['Credit'] := IfThen(iPDPar.Platba.Kredit,'1','0'); //pro WebApi nefungovalo dob¯e
    ...
    if Assigned(iPDPar.Doklad) then
      ...
      begin
        newBo.rowItem['PDocumentType'] := iPDPar.Doklad.DocumentType; // nap¯. 03, 10, 61
      ...
      if not(iPDPar.pdparTyp = 'VoipKredit') AND not(iPDPar.pdparTyp = 'InternetKredit') then  //tyto podmÌnky nejsou nutnÈ, Abra by Firm_ID p¯ebila hodnotou z fa, nicmÈnÏ s VoipKredit a InternetKredit pracujeme v algoritmu nÌûe a p·rujeme na faktury, tedy Firm_ID nebudeme zasÌlat
        newBo.rowItem['Firm_ID'] := getFirmIdKdyzNeniDoklad(iPDPar.Platba); //kdyû m· text platby na zaË·tku Abra kÛd, najde se firma. Jinak se d· '3Y90000101' (DES), aby tam nebyl default "drobn˝ n·kup"
    ...
    begin
      ...
      if faId = '' then
        ...
        newBo.rowItem['PDocumentType'] := '03'; // je to vûdy faktura
        newBo.rowItem['PDocument_ID'] := faId;
        resultMesg := resultMesg + 'Vytvo¯ena fa za Voip Kredit - '  + faId + sLineBreak;
      end;
    end;
    ...
    begin
      ...
      if faId = '' then
        ...
        newBo.rowItem['PDocumentType'] := '03'; // je to vûdy faktura
        newBo.rowItem['PDocument_ID'] := faId;
        resultMesg := resultMesg + 'Vytvo¯ena fa za Internet Kredit - '  + faId + sLineBreak;
      end;
    end;


    if iPDPar.Platba.Debet then
      newBo.rowItem['VarSymbol'] := iPDPar.Platba.VS; //pro debety aby vûdy z˘stal VS
    ...
    if (iPDPar.Platba.cisloUctuVlastni = '4003292157000000') AND iPDPar.Platba.kredit then begin //PayU platba, ruöÌme penÌze na cestÏ
  ...
  try begin
    ...
    if newBo.WriteResult.isOk then begin
      ...
      newBo.WriteResult.Messg := resultMesg + 'Z·pis v˝pisu do ABRA pro ˙Ëet ' + self.Vypis.abraBankaccount.name + '.'
        + ' »Ìslo novÈho v˝pisu je ' + newBo.getCreatedBoItem('displayname') + ' (' + newBo.getCreatedBoItem('id') + ')';
    end;

...
DELIMITER //
CREATE PROCEDURE get_monthly_invoicing_cu_by_vsrange(
    ...
    IN in_return_nonvoip BOOLEAN,
    IN in_return_voip BOOLEAN
)
BEGIN
SELECT DISTINCT 
...
WHERE 
	...
	AND (
		(in_return_nonvoip AND (IF (voip_cnt.voip_count, 1, 0)) = 0) 
		OR (in_return_voip AND (IF (voip_cnt.voip_count, 1, 0)) = 1)
	) 		
ORDER BY cu.variable_symbol;
END //
DELIMITER ;

CALL get_monthly_invoicing_cu_by_vsrange('2023-05-01','2023-05-31','1','7961230002',true,true)

DROP PROCEDURE get_monthly_invoicing_cu_by_vsrange

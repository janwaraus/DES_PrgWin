...
type
  ...
  private
    ...
    procedure FakturaAbraAA(Radek: integer);
  public
...
// vytvoøí se hlavièka faktury
    ...
    FData.ValueByName('DocQueue_ID') := VarToStr(globalAA['abraIiDocQueue_Id']);
    FData.ValueByName('Period_ID') := VarToStr(globalAA['abraIiPeriod_Id']);
    ...
    FData.ValueByName('CreatedBy_ID') := MyUser_Id; // TODO bylo jen User_Id ale to bylo prázdné, bez zalogování
...
// cena za tarif
      if (FieldByName('Tarifni').AsInteger = 1) then begin
        ...
        if not SmlouvaVoIP then begin                    // pøipojení k Internetu
          ...
          end else FRow.ValueByName('VATIndex_ID') := VarToStr(globalAA['abraVatIndex_Id']);
          ...
          if HovorneVoIP > 0 then begin                                      // hovorné
            ...
            FRow.ValueByName('Division_ID') := '1000000101';
            FRow.ValueByName('VATRate_ID') := VarToStr(globalAA['abraVatRate_Id']);
            FRow.ValueByName('VATIndex_ID') := VarToStr(globalAA['abraVatIndex_Id']);
            FRow.ValueByName('VATRate') := IntToStr(globalAA['abraVatRate']);
            FRow.ValueByName('IncomeType_ID') := '2000000000';
            FRow.ValueByName('RowType') := '1';
...
procedure TdmFaktura.FakturaAbraAA(Radek: integer);
// pro mìsíc a rok zadaný v aseMesic a aseRok vytvoøí fakturu za pøipojení k internetu a za VoIP
// 27.1.17 celé pøehlednìji
var
  Firm_Id,
